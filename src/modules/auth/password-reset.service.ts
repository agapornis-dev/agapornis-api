...
export class PasswordResetService {
  ...
  async consume(token: string) {
    const tokenHash = this.hash(String(token || ''));
    ...
    return this.database.transaction(async tx => {
      ...
      const rows = await tx.query(
        'SELECT user_id, expires_at, consumed_at FROM password_reset_tokens WHERE token_hash = ? FOR UPDATE',
        [tokenHash],
      );
      const record = rows[0];
      if (!record || record.consumed_at || Date.parse(String(record.expires_at)) <= Date.now()) return undefined;
      await tx.query('UPDATE password_reset_tokens SET consumed_at = ? WHERE token_hash = ?', [now, tokenHash]);

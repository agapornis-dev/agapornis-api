...
async function main() {
  ...
  assert.ok(upgradeSql.some(sql => /ALTER TABLE server_schedules ADD COLUMN IF NOT EXISTS consecutive_failures/i.test(sql)),
    'existing server_schedules tables must add consecutive_failures');
